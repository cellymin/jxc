...
function OSAdminAutoLoad($classname)
{
    ...
    $filepath = ADMIN_BASE_LIB . 'barcode/' . $classname . '.php';
    if (file_exists($filepath)) {
        return include $filepath;
    }

    $filepath = ADMIN_BASE_LIB . 'barcode/drawer/' . $classname;
    if (file_exists($filepath)) {
        return include $filepath;
    }

    $filepath = ADMIN_BASE_LIB . 'barcode/' . $classname . '.barcode.php';
    if (file_exists($filepath)) {
        return include $filepath;
    }
...
$no_need_login_page = array("/block.php", "/panel/login.php", "/panel/logout.php","/storage/depot.php");
...
Template::assign('userId', $_SESSION[UserSession::SESSION_NAME]['user_id']);
...
//表单随机字符串
if (Common::isGet()) {
    if(!isset($_SESSION[UserSession::SESSION_NAME]['form_nonceStr']) && empty($_SESSION[UserSession::SESSION_NAME]['form_nonceStr'])){
        Common::resetNonceStr();
    }else{
        $nonceStr = $_SESSION[UserSession::SESSION_NAME]['form_nonceStr'];
